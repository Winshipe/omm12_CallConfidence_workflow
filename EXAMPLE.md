...
Adjust the config file to use your mobileOG by replacing "resources/mmseqs/placeholder" with your actual path 

I included a copy of the *A. muris* genome from NCBI (Accession CP065321.1) in resources and the supplied `config.yaml` file already has everything else filled out to run for *A. muris* with a mutant allele frequency of 0.5. 

### 3.  Run the pipeline
All that's left is to run the pipeline with the following command
